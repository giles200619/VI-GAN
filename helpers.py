...
# Helper Functions
##########################################################
def get_norm_layer(filters, norm_type='instance'):
    if norm_type == 'batch':
        return nn.BatchNorm2d(filters, affine=True)
    elif norm_type == 'instance':
        return nn.InstanceNorm2d(filters, affine=False, track_running_stats=False)
    else:
...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch - opt.niter) / float((opt.num_epochs - opt.niter) + 1)
            return lr_l
